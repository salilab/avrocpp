...
static vector<string>::const_iterator skipCalls(
    Scanner& sc, Decoder& d, vector<string>::const_iterator it, bool isArray) {
  char end = isArray ? ']' : '}';
  int level = 0;
  while (!sc.isDone()) {
    char c = sc.advance();
    ...
    switch (c) {
      ...
      case 'b':
        ...
        if (containerStack.size() >= skipLevel) {
          ...
          BOOST_CHECK_EQUAL_COLLECTIONS(b1.begin(), b1.end(), b2.begin(),
                                        b2.end());
        }
        ++it;
        break;
...
static const TestData data[] = {
    {"\"null\"", "N", 1}, {"\"boolean\"", "B", 1}, {"\"int\"", "I", 1},
    {"\"long\"", "L", 1}, {"\"float\"", "F", 1}, {"\"double\"", "D", 1},
    {"\"string\"", "S0", 1}, {"\"string\"", "S10", 1}, {"\"bytes\"", "b0", 1},
    {"\"bytes\"", "b10", 1},
    ...
    {"{\"type\":\"array\", \"items\":"
     ...
     "\"type\":[\"null\", \"int\"]}]}}",
     "[c2sLU0NsLU1I]", 2},
    {"[\"boolean\", \"null\" ]", "U0B", 1}, {"[\"int\", \"null\" ]", "U0I", 1},
    {"[\"long\", \"null\" ]", "U0L", 1}, {"[\"float\", \"null\" ]", "U0F", 1},
    {"[\"double\", \"null\" ]", "U0D", 1},
    {"[\"string\", \"null\" ]", "U0S10", 1},
    {"[\"bytes\", \"null\" ]", "U0b10", 1}, {"[\"null\", \"int\"]", "U0N", 1},
    {"[\"boolean\", \"int\"]", "U0B", 1}, {"[\"boolean\", \"int\"]", "U1I", 1},
...
static const TestData4 data4[] = {
    ...
    {"{\"type\":\"record\",\"name\":\"r\",\"fields\":["
     ...
     "{\"name\":\"f2\", \"type\":\"string\"},"
     "{\"name\":\"f3\", \"type\":\"int\"}]}",
     "S10S10IS10S10I", {"s1", "s2", "100", "t1", "t2", "200", NULL},
     "{\"type\":\"record\",\"name\":\"r\",\"fields\":["
     "{\"name\":\"f1\", \"type\":\"string\" },"
     "{\"name\":\"f2\", \"type\":\"string\"}]}",
     "RS10S10RS10S10", {"s1", "s2", "t1", "t2", NULL}, 1},
    ...
    {"{\"type\":\"record\",\"name\":\"r\",\"fields\":["
     ...
     "{\"name\":\"f3\", \"type\":\"float\"}"
     "]}",
     "BIU0IF", {"1", "100", "121", "10.75", NULL},
     "{\"type\":\"record\",\"name\":\"r\",\"fields\":["
     "{\"name\":\"f0\", \"type\":\"boolean\"},"
     "{\"name\":\"f1\", \"type\":\"long\"},"
     "{\"name\":\"f3\", \"type\":\"double\"}]}",
     "BLD", {"1", "100", "10.75", NULL}, 1}, };

#define COUNTOF(x) sizeof(x) / sizeof(x[0])
#define ENDOF(x) (x) + COUNTOF(x)

#define ADD_TESTS(testSuite, Factory, testFunc, data)           \
  testSuite.add(BOOST_PARAM_TEST_CASE(&testFunc<Factory>, data, \
                                      data + COUNTOF(data)))
...
void add_tests(boost::unit_test::test_suite& ts) {
  ...
  ADD_TESTS(ts, BinaryEncoderResolvingDecoderFactory, testCodecResolving,
            data3);
  ADD_TESTS(ts, BinaryEncoderResolvingDecoderFactory, testCodecResolving2,
            data4);
  ADD_TESTS(ts, ValidatingEncoderResolvingDecoderFactory, testCodecResolving2,
            data4);
