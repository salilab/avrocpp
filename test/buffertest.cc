...
void TestGrow() {
  BOOST_MESSAGE("TestGrow");
  {
    ...
    BOOST_CHECK_EQUAL(
        ob.size(), kDefaultBlockSize + kDefaultBlockSize / 2 + reserveSize + 1);
...
void TestDiscard() {
...

  {
    ...
    try {
      ...
      std::cout << "Intentionally triggered exception: " << e.what()
                << std::endl;
    }
...
void TestIterator() {
  BOOST_MESSAGE("TestIterator");
  {
    ...
    size_t acc = 0;
    for (OutputBuffer::const_iterator iter = ob.begin(); iter != ob.end();
         ++iter) {
      acc += iter->size();
    }
