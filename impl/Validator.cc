  ...
  if (compoundStarted_) {
    ...
    if (count_ < static_cast<int64_t>(node->leaves())) {
      ...
      throw Exception(boost::format(
                          "Union selection out of range, got %1%,"
                          " expecting 0-%2%") %
...
void Validator::doAdvance() {
  ...
  static const AdvanceFunc funcs[] = {
      ...
      0,                            // bool
      0,                            // null
      &Validator::countingAdvance,  // Record is treated like counting with
                                    // count == 1
...
void Validator::setupFlag(Type type) {
  // use flags instead of strictly types, so that we can be more lax about the
  // type
  // (for example, a long should be able to accept an int type, but not vice
  // versa)
  static const flag_t flags[] = {typeToFlag(AVRO_STRING) |
                                     typeToFlag(AVRO_BYTES),
                                 typeToFlag(AVRO_STRING) |
                                     typeToFlag(AVRO_BYTES),
                                 typeToFlag(AVRO_INT),
                                 typeToFlag(AVRO_INT) | typeToFlag(AVRO_LONG),
                                 typeToFlag(AVRO_FLOAT),
                                 typeToFlag(AVRO_DOUBLE), typeToFlag(AVRO_BOOL),
                                 typeToFlag(AVRO_NULL), typeToFlag(AVRO_RECORD),
...
bool Validator::getCurrentRecordName(std::string &name) const {
  ...
  if (!compoundStack_.empty() &&
      (isPrimitive(nextType_) || nextType_ == AVRO_RECORD)) {
...
bool Validator::getNextFieldName(std::string &name) const {
  bool found = false;
  name.clear();
  int idx = isCompound(nextType_) ? compoundStack_.size() - 2
                                  : compoundStack_.size() - 1;
  if (idx >= 0 && compoundStack_[idx].node->type() == AVRO_RECORD) {
