...
const map<string, Entity>::const_iterator findField(
    const Entity& e, const map<string, Entity>& m, const string& fieldName) {
...
static NodePtr makeRecordNode(const Entity& e, const Name& name,
                              const map<string, Entity>& m, SymbolTable& st,
                              const string& ns) {
  ...
  return NodePtr(
      new NodeRecord(asSingleAttribute(name), fieldValues, fieldNames));
}

static NodePtr makeEnumNode(const Entity& e, const Name& name,
                            const map<string, Entity>& m) {
...
static NodePtr makeFixedNode(const Entity& e, const Name& name,
                             const map<string, Entity>& m) {
  ...
  return NodePtr(new NodeFixed(asSingleAttribute(name), asSingleAttribute(v)));
}
...
static NodePtr makeArrayNode(const Entity& e, const map<string, Entity>& m,
  ...
  return NodePtr(
      new NodeArray(asSingleAttribute(makeNode(it->second, st, ns))));
}
...
static NodePtr makeMapNode(const Entity& e, const map<string, Entity>& m,
  ...
  return NodePtr(new NodeMap(asSingleAttribute(makeNode(it->second, st, ns))));
}

static Name getName(const Entity& e, const map<string, Entity>& m,
                    const string& ns) {
...
static NodePtr makeNode(const Entity& e, const map<string, Entity>& m,
  ...
  if (NodePtr result = makePrimitive(type)) {
    return result;
  } else if (type == "record" || type == "error" || type == "enum" ||
             type == "fixed") {
