...
Production ValidatingGrammarGenerator::doGenerate(
    const NodePtr& n, map<NodePtr, shared_ptr<Production> >& m) {
  switch (n->type()) {
    case AVRO_NULL:
    ...
    case AVRO_MAP: {
      ...
      Symbol r[] = {Symbol::mapEndSymbol(), Symbol::repeater(v, false),
...
struct DummyHandler {
  size_t handle(const Symbol& s) { return 0; }
};
