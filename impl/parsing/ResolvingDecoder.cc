...
class ResolvingGrammarGenerator : public ValidatingGrammarGenerator {
  ...
  Production getWriterProduction(
      const NodePtr& n, const map<NodePtr, shared_ptr<Production> >& m2);
...
Production ResolvingGrammarGenerator::getWriterProduction(
    const NodePtr& n, const map<NodePtr, shared_ptr<Production> >& m2) {
  const NodePtr& nn = (n->type() == AVRO_SYMBOLIC)
                          ? static_cast<const NodeSymbolic&>(*n).getNode()
                          : n;
...
Production ResolvingGrammarGenerator::resolveUnion(
  ...
  Symbol r[] = {Symbol::alternative(v), Symbol::writerUnionAction()};
  ...
  if (writerType == readerType) {
    switch (writerType) {
      ...
      case AVRO_ARRAY: {
        ...
        Symbol r[] = {Symbol::arrayEndSymbol(),
                      Symbol::repeater(doGenerate(writer->leafAt(0),
                                                  reader->leafAt(0), m, m2),
                                       p, true),
                      Symbol::arrayStartSymbol()};
        return Production(r, r + 3);
      }
      case AVRO_MAP: {
        Production v = doGenerate(writer->leafAt(1), reader->leafAt(1), m, m2);
        v.push_back(Symbol::stringSymbol());

        Production v2 = getWriterProduction(writer->leafAt(1), m2);
        v2.push_back(Symbol::stringSymbol());

        Symbol r[] = {Symbol::mapEndSymbol(), Symbol::repeater(v, v2, false),
    ...
    switch (readerType) {
        ...
        if (writerType == AVRO_INT || writerType == AVRO_LONG) {
          return Production(
              1, Symbol::resolveSymbol(
                     writerType == AVRO_INT ? Symbol::sInt : Symbol::sLong,
                     Symbol::sFloat));
        }
        break;
      case AVRO_DOUBLE:
      ...
      case AVRO_UNION: {
        ...
        if (j >= 0) {
          ...
          Symbol r[] = {Symbol::unionAdjustSymbol(j, p), Symbol::unionSymbol()};
          return Production(r, r + 2);
        }
      } break;
      case AVRO_NULL:
...
class ResolvingDecoderImpl : public ResolvingDecoder {
 ...
 public:
  ResolvingDecoderImpl(const ValidSchema& writer, const ValidSchema& reader,
      ...
      : base_(base),
        handler_(*base_),
        parser_(ResolvingGrammarGenerator().generate(reader, writer), &(*base_),
                handler_) {}
};
...
float ResolvingDecoderImpl<P>::decodeFloat() {
  ...
  return k == Symbol::sInt ? base_->decodeInt() : k == Symbol::sLong
                                                      ? base_->decodeLong()
                                                      : base_->decodeFloat();
}
...
ResolvingDecoderPtr resolvingDecoder(const ValidSchema& writer,
  ...
  return make_shared<parsing::ResolvingDecoderImpl<
      parsing::SimpleParser<parsing::ResolvingDecoderHandler> > >(writer,
                                                                  reader, base);
}
