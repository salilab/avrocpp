...
class AVRO_DECL OutputBuffer {
 ...
 public:
    ...
    if (size) {
      if (size > freeSpace()) {
        throw std::length_error(
            "Impossible to write more data than free space");
      }
  ...
  /**
   * Discard the specified number of bytes from this data, starting at the
   *beginning.
...
inline void toIovec(BufferType &buf, std::vector<struct iovec> &iov) {
  ...
  for (int i = 0; i < chunks; ++i) {
    iov[i].iov_base =
        const_cast<typename BufferType::data_type *>(iter->data());
