...
class Chunk {
 ...
 public:
  ...
  data_type *tellWritePos() const { return writePos_; }

  /// Tell the position of the first byte containing valid data.
  const data_type *tellReadPos() const { return readPos_; }
  ...
  size_type dataSize() const { return (writePos_ - readPos_); }

  /// Tell how many bytes this chunk has available to write to.
  size_type freeSize() const { return (endPos_ - writePos_); }

  /// Tell how many bytes of data this chunk can hold (used and free).
  size_type capacity() const { return (endPos_ - readPos_); }
 ...
 *
 * Internally, BufferImpl keeps two lists of chunks, one list consists entirely
 *of
...
class BufferImpl : boost::noncopyable {
  ...
  void postWrite(size_type size) {
    ...
    // update the states of both readChunks_ and writeChunks_ to indicate that
    // they are
 ...
 public:
  ...
  BufferImpl() : freeSpace_(0), size_(0) {}
  ...
  size_type freeSpace() const { return freeSpace_; }
  ...
  ChunkList::const_iterator beginRead() const { return readChunks_.begin(); }

  /// Return the chunk avro's end iterator for reading.
  ChunkList::const_iterator endRead() const { return readChunks_.end(); }

  /// Return the chunk avro's begin iterator for writing.
  ChunkList::const_iterator beginWrite() const { return writeChunks_.begin(); }

  /// Return the chunk avro's end iterator for writing.
  ChunkList::const_iterator endWrite() const { return writeChunks_.end(); }
  ...
  void append(const BufferImpl &src) {
    std::copy(src.readChunks_.begin(), src.readChunks_.end(),
              std::back_inserter(readChunks_));
    size_ += src.size_;
  }
  ...
  /// Remove the specified amount of data from the chunks, starting at the
  /// front.
  ...
  void copyData(BufferImpl &dest, ChunkList::const_iterator iter,
                size_type offset, size_type bytes) const {
