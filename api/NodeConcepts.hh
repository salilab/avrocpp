...
struct SingleAttribute {
  ...
  void add(const Attribute &attr) { attr_ = attr; }
...
struct MultiAttribute {
  ...
  void add(const Attribute &attr) { attrs_.push_back(attr); }

  const Attribute &get(size_t index = 0) const { return attrs_.at(index); }

  Attribute &get(size_t index) { return attrs_.at(index); }
