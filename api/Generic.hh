...
class AVRO_DECL GenericDatum {
 ...
 public:
  ...
  /// std::vector<uint8_t>.
  GenericDatum(const std::vector<uint8_t>& v) : type_(AVRO_BYTES), value_(v) {}
...
class AVRO_DECL GenericUnion : public GenericContainer {
 ...
 public:
   ...
   */
  Type type() const { return datum_.type(); }
...
class AVRO_DECL GenericRecord : public GenericContainer {
 ...
 public:
   ...
   */
  const GenericDatum& fieldAt(size_t pos) const { return fields_[pos]; }

  /**
   * Returns the reference to the field at the given position \p pos,
   * which can be used to change the contents.
   */
  GenericDatum& fieldAt(size_t pos) { return fields_[pos]; }
...
class AVRO_DECL GenericEnum : public GenericContainer {
 ...
 public:
   ...
   */
  size_t set(const std::string& symbol) { return value_ = index(symbol); }
   ...
   */
  const std::string& symbol() const { return schema()->nameAt(value_); }
};
...
inline Type AVRO_DECL GenericDatum::type() const {
  return (type_ == AVRO_UNION) ? boost::any_cast<GenericUnion>(&value_)->type()
                               : type_;
}
